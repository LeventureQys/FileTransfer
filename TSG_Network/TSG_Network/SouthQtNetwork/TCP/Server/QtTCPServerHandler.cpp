...
bool QtTCPServerHandler::Open(const quint16 port)
{
	...
	{
		qInfo() << QStringLiteral("监听端口失败：") + QString::number(port);
		return false;
	}
...
void QtTCPServerHandler::RecvClnMsg()
{
	...
	ReadTCPMsg(pTcpSocket, pClnInfo->nRecvedMsgLen, [&](const QByteArray& bytes) {
		qInfo() << QStringLiteral("qtnetdll收到来自%1的TCP：%2").arg(qsPeerAddr).arg(QString::fromLocal8Bit(bytes));
...
void QtTCPServerHandler::NewConn()
{
	...
	// 
	qInfo() << QStringLiteral("qtnetdll新的连接：(%1)").arg(pClnTcpSocket->peerAddress().toString());
...
void QtTCPServerHandler::NewDisConn()
{
	...
	qInfo() << QStringLiteral("qtnetdll新的断开：(%1, %2)").arg(qsIP).arg(qsErr);
